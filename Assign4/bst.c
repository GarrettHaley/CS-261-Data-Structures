  ...
  */
  ...
  while (tree != 0) {
    if (val == tree->val->number)
      return tree->val; /* Return true if val found. */
    if (val < tree->val->number)
      tree = tree->left; /* Otherwise, go to the left */
    else tree = tree->right; /* or right, depending on val. */
  }
...
int secondLargestNode(struct Node *tree){
	int secondLargestNode;
       struct Node *previousVal = tree;
       struct Node *currentVal = tree;
        ...
        if (currentVal->left != NULL) {
            ...
            while (currentVal->right != NULL)
                currentVal = currentVal->right;
            secondLargestNode = currentVal->val->number;
        }
        else{
            if (currentVal == tree && previousVal == tree)
                secondLargestNode = -1;
           else
                secondLargestNode = previousVal->val->number;
             }
...
void  flattenPreorder(struct Node *tree, struct cirListDeque * d){
  /* Performs a preorder traversal of tree and appends nodes in the
	...
	if(tree == NULL)
		return;
	addFrontCirListDeque(d,tree->val);
